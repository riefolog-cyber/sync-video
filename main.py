...
from semantic_sync import free_order_segments_from_words, semantic_timeline_from_words
from timeline import (
    detect_flow_from_words,
    extract_slide_anchors,
    reconcile_timeline,
)
from transcription import generate_compact_transcript, transcribe_with_whisper
...
def find_audio_file(directory: Path) -> Path | None:
    ...
    candidates = [
        ...
        if f.suffix.lower() in extensions and f.name != "temp_vosk_audio.wav"
    ]
    if not candidates:
        return None
...
def _print_timing(t_ocr: float, t_vosk: float, t_sync: float,
                  t_video: float, t_total: float) -> None:
    """Stampa il riepilogo dei tempi di ogni fase."""
    log.info("\n" + "─" * 50)
    log.info(" ⏱️  RIEPILOGO TEMPI")
    log.info("─" * 50)
    log.info("   OCR / Slide   │ %s", _format_time(t_ocr))
    log.info("   Trascrizione  │ %s", _format_time(t_vosk))
...
def main(argv: list | None = None) -> None:
    ...
    t_ocr = t_vosk = t_sync = t_video = 0.0
    ...
    if not args.no_cache:
        ...
        if (cached and "slide_files" in cached and "slide_texts" in cached
            ...
            log.info("1. [CACHE] Slide OCR recuperate dalla cache (%d slide).",
                     len(slide_files))
    ...
    if slide_files is None or slide_texts is None:
        ...
        slide_files, slide_texts = extract_slides_text_ocr(
            pdf_path, args.slides_dir,
            lang=args.lang,
            dpi=args.dpi,
            workers=args.ocr_workers,
        )
        if not args.no_cache:
            _save_cache(cache_key_slides, {
    ...
    try:
        ...
        t_phase_start = time.time()
        temp_wav = BASE_DIR / "temp_vosk_audio.wav"
        ...
        if transcript is None:
            if args.transcriber == "whisper":
                transcript, words_raw = transcribe_with_whisper(
                    audio_path,
                    language=args.lang,
                    model_size=args.whisper_model,
                )
            else:
                transcript, words_raw = generate_compact_transcript(
                    audio_path,
                    vosk_model_name=args.vosk_model,
                    vosk_model_dir=args.vosk_model_path,
                    lang=args.lang,
                    temp_wav=temp_wav,
                    sample_rate=DEFAULT_AUDIO_SAMPLE_RATE,
                    chunk_bytes=DEFAULT_VOSK_CHUNK_BYTES,
                )
            if not args.no_cache:
                # Fix A: salva anche le parole raw per estrazione deterministica
                _save_cache(cache_key_transcript, {
                    "transcript": transcript,
                    "words_raw": words_raw,
                })

        t_vosk = time.time() - t_phase_start
        ...
        slide_ids: list[int]
        if flow == "free":
            log.info("3. Selezione libera: le slide seguono il contenuto del podcast, "
                     "senza vincolo di ordine.")
            ...
            for i, seg in enumerate(segments):
                log.info(
                    ...
                    slide_ids[i], float(seg["start"]), float(seg["end"]), durations[i],
                )
            ...
            if semantic_anchors:
                log.info(
                    "3. [Ancore] %d ancore 'slide N' coerenti usate per vincolare "
                    "l'allineamento semantico.",
            ...
            if (args.llm != "off" and semantic_anchors
                    and len(semantic_anchors) < total_slides - 1):
                endpoints = endpoints_for(args.llm)
                if args.llm_model:
                    for ep in endpoints:
                        ep["model"] = args.llm_model
            ...
            if timeline is None:
                _abort("Sincronizzazione semantica fallita: nessuna timeline "
                       "generabile da slide + trascrizione.")
            ...
            try:
                durations = reconcile_timeline(
                    timeline, total_slides, total_duration,
                )
        ...
        thin = [
            slide_ids[i] for i, d in enumerate(durations)
            if d < 2 * args.semantic_min_duration
        ]
        if thin:
            log.warning(
        ...
        if args.preview:
            ...
            for i, dur in enumerate(durations):
                ...
                log.info("   Slide %2d: %6.1fs ─ %6.1fs (%6.1fs) %s",
                         slide_ids[i], start, end, dur, bar)
            log.info("=" * 70)
        ...
        if args.dry_run:
            ...
            _print_timing(t_ocr, t_vosk, t_sync, 0.0, t_total)
            log.info("\n" + "=" * 70)
        ...
        t_phase_start = time.time()
        build_video(
            slide_files, durations, audio_clip, args.output_video,
        ...
        try:
            ...
            if abs(out_dur - expected) > 1.0:
                log.warning(
                    "   ⚠️ Durata video (%.1fs) diversa da audio+buffer (%.1fs): "
                    "possibili troncamenti.",
                    out_dur, expected,
                )
            else:
                log.info(
        ...
        # --- Riepilogo finale ---
        t_total = time.time() - t_total_start
        _print_timing(t_ocr, t_vosk, t_sync, t_video, t_total)
        ...
        # Pulizia WAV temporaneo (anche se già rimosso da transcription)
        # Usa try/except per gestire il caso in cui il file sia ancora bloccato da ffmpeg
        try:
            (BASE_DIR / "temp_vosk_audio.wav").unlink(missing_ok=True)
        except PermissionError:
            log.debug("   temp_vosk_audio.wav ancora in uso (ffmpeg), saltata rimozione.")
